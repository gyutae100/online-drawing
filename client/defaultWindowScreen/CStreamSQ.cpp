...
int	CStreamSQ::Put(char *in_data, int sz_byte_data) {
	...
	if (_idx_front > next_idx_rear) {
		...
		if (sz_byte_data > sz_left_terminal_partition) {
			...
			MoveRearIdx(sz_left_terminal_partition);
...
void CStreamSQ::DisplayInfo1(void) {
	...
	char info[1000];
	sprintf_s(info, "title put / front = %d / rear = %d /  use = %d / free = %d /", _idx_front, _idx_rear, GetSizeOfUse(), GetSizeOfFree());
	//char info2[1000];
	//strcpy_s(info2, _ring_buffer);
	//info2[5] = NULL;
	//strcat_s(info, info2);

	if (_idx_front > _total_sz_byte_ring_buffer)
	{
		int tmp = 3;
...
void CStreamSQ::DisplayInfo2(void) {
	...
	char info[1000];
	sprintf_s(info, "title get / front = %d / rear = %d /  use = %d / free = %d /", _idx_front, _idx_rear, GetSizeOfUse(), GetSizeOfFree());
